...
exports.getBCGPortfolioData = async (req, res) => {
  try {
    // Serve from pre-computed JSON (real data with BCG_CUADRANTE assigned by KMeans)
    if (fs.existsSync(DATA_PATH)) {
      const raw = JSON.parse(fs.readFileSync(DATA_PATH, 'utf-8'));
      // Return full portfolio object (includes cuadrantes summary + productos array)
      return res.json(raw);
    }
    // Fallback: query Snowflake directly (no BCG_CUADRANTE, frontend will compute)
    console.log("JSON not found, fetching from Snowflake...");
    const data = await getBCGData();
    res.json({ productos: data, total_productos: data.length });
  } catch (error) {
