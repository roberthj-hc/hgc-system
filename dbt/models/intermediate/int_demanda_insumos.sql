...
with movimientos_salida as (
    ...
    from {{ ref('stg_mysql__movimientos_inventario') }}
...
lead_time_compras as (
    ...
    from {{ ref('stg_mysql__detalle_oc') }} doc
    join {{ ref('stg_mysql__ordenes_compra') }} oc on doc.id_oc_nk = oc.id_oc_nk
    group by 1
),
...
recetas as (
    ...
    from {{ ref('stg_mysql__recetas_bom') }}
    group by 1
)
