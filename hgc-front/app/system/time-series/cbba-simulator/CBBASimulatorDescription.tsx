...
export function CBBASimulatorDescription() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      const [histRes, predRes] = await Promise.all([
        fetch(`http://localhost:5000/api/simulator/historical?branchId=${selectedBranch}`).catch(() => null),
        fetch(`http://localhost:5000/api/simulator/simulate?branchId=${selectedBranch}&mkt=${marketing}&price=${price}&employees=${employees}`).catch(() => null)
      ]);

      // histJson puede ser [] si Snowflake no está disponible
      const histJson: any[] = histRes ? await histRes.json().catch(() => []) : [];
      const predJson = predRes ? await predRes.json().catch(() => null) : null;

      // Si predJson es un objeto con error, lanzar excepcion
      if (predJson && !Array.isArray(predJson) && predJson.error) {
        throw new Error(predJson.error);
      }

      const predArray: any[] = Array.isArray(predJson) ? predJson : [];

      // Tomar los ultimos 90 dias de historico
      const last90 = (Array.isArray(histJson) ? histJson : []).slice(-90);
      ...
      if (combined.length > 0 && predArray.length > 0) {
      ...
      console.error("Error al cargar simulador:", e);
      setChartData([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <SmartButtons
        module="Simulador de Apertura CBBA"
        data={{ sucursalBase: selectedBranch, marketing, precioCombo: price, empleados: employees, veredicto: rentable ? 'Recomendado' : 'Alto Riesgo', ventaEstimada: predData.length > 0 ? predData[predData.length - 1]?.prediccion : null }}
      />
