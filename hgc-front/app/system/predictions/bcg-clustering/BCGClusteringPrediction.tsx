...
interface ProductoBCG {
  ...
  FEATURE_SHARE_REVENUE_PCT?: number;
  cuadrante?: string;
  color?: string;
  emoji?: string;
}
...
export default function BCGClusteringPrediction() {
  ...
  // ── Cargar portafolio (desde JSON pre-computado con datos reales) ──────────
  const fetchPortfolio = useCallback(async () => {
    ...
    try {
      ...
      // Handle both formats: flat array OR {productos, cuadrantes, ...}
      const prods: ProductoBCG[] = Array.isArray(json) ? json : (json.productos || []);
      if (!Array.isArray(json) && json.total_productos) setPortfolioStats(json);
      // If BCG_CUADRANTE already present (from pre-computed JSON), use it directly
      const enriched = prods.map((p: any) => {
        const cuad: string = p.BCG_CUADRANTE || "";
        if (cuad && BCG_QUADRANTS[cuad as keyof typeof BCG_QUADRANTS]) {
          const q = BCG_QUADRANTS[cuad as keyof typeof BCG_QUADRANTS];
          const emoji = cuad === "Estrella" ? "⭐" : cuad === "Vaca Lechera" ? "🐄" : cuad === "Interrogante" ? "❓" : "🐕";
          return { ...p, cuadrante: cuad, color: q.color, emoji };
        }
        return p;
      });
      const hasQuadrant = enriched.every((p: any) => p.cuadrante);
      setPortfolio(hasQuadrant ? enriched : assignQuadrantLocally(enriched));
    } catch (err: any) {
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      ...
      <SmartButtons module="BCG Portfolio" data={{ totalProductos: portfolio.length, cuadrantes: { estrellas: countByQ("Estrella"), vacas: countByQ("Vaca Lechera"), interrogantes: countByQ("Interrogante"), perros: countByQ("Perro") }, gananciaTotal: totalGanancia }} />
      ...
      <div className="grid gap-5 md:grid-cols-2 mt-2">
        ...
        <Card className="shadow-sm border-slate-200 dark:border-slate-800">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              ...
              Ganancia Real por Cuadrante
            </CardTitle>
            <CardDescription className="text-xs">
              {portfolioStats
                ? `${portfolioStats.pareto_n80} de ${portfolioStats.total_productos} productos generan el 80% de la ganancia · Total: Bs ${portfolioStats.ganancia_total?.toLocaleString("es-BO", { maximumFractionDigits: 0 })}`
          ...
          <CardContent className="h-[200px] pr-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={[
                { cuad: "🐄 Vaca Lechera", ganancia: gananciaByQ("Vaca Lechera") / 1e6 },
                { cuad: "🐕 Perro",        ganancia: gananciaByQ("Perro") / 1e6 },
                { cuad: "❓ Interrogante", ganancia: gananciaByQ("Interrogante") / 1e6 },
                { cuad: "⭐ Estrella",     ganancia: gananciaByQ("Estrella") / 1e6 },
              ]} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal={false} opacity={0.3} />
                <XAxis type="number" tickFormatter={v => `Bs ${v.toFixed(0)}M`} fontSize={10} />
                <YAxis dataKey="cuad" type="category" width={110} fontSize={11} />
                <RechartsTooltip contentStyle={{ borderRadius: "8px", fontSize: "12px" }}
                  formatter={(v: any) => [`Bs ${(v as number).toFixed(1)}M`, "Ganancia"]} />
                <Bar dataKey="ganancia" radius={[0, 4, 4, 0]} barSize={22}>
                  {["#4CAF9A","#E05C5C","#5B8DEF","#F4C542"].map((c, i) => <Cell key={i} fill={c} />)}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        ...
        <Card className="shadow-sm border-slate-200 dark:border-slate-800">
          <CardHeader className="pb-2 bg-slate-50 dark:bg-slate-900/50">
            ...
            <CardDescription className="text-xs">
              Campeón: <span className="font-bold">KMeans (4 clusters)</span> · Datos reales Snowflake
            </CardDescription>
          </CardHeader>
          <CardContent className="h-[200px] pr-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart layout="vertical" data={[
                { metric: "Silhouette Score",        value: 44.5, note: "KMeans real" },
                { metric: "Calinski-Harabasz (norm)",value: 67,   note: "Separación clusters" },
                { metric: "Pareto (% prod → 80% gan)",value: 33,  note: "3 de 9 productos" },
              ]} margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" horizontal vertical={false} opacity={0.3} />
                <XAxis type="number" tickFormatter={v => `${v}%`} fontSize={11} domain={[0, 100]} />
                <YAxis dataKey="metric" type="category" width={175} fontSize={10} />
                <RechartsTooltip contentStyle={{ borderRadius: "8px", fontSize: "12px" }}
                  formatter={(v: any, _: any, p: any) => [`${v}% — ${p.payload.note}`, ""]} />
