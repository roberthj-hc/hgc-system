...
const predictQ = (intercept: number, elasticity: number, price: number): number => {
    ...
    >
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 bg-[#020617] text-slate-50 min-h-screen">
          ...
          <SmartButtons module="Optimizador de Precios" data={{ productoSeleccionado: selectedProduct, elasticidad: coefficients?.ELASTICIDAD, precioSugerido: suggestedPrice, statsIngresos: stats }} />
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <Card className="bg-slate-900/50 border-slate-800 backdrop-blur-sm">
              ...
              <CardContent>
                <div className={`text-2xl font-bold ${(stats?.extraSurplus ?? 0) >= 0 ? "text-emerald-400" : "text-rose-400"}`}>
                  {stats ? `${(stats.extraSurplus ?? 0) >= 0 ? '+' : ''}Bs. ${(stats.extraSurplus ?? 0).toLocaleString(undefined, {maximumFractionDigits: 0})}` : "Bs. 0"}
                </div>
