...
export function EfficiencyMonitorClient() {
  const [fullData, setFullData] = useState<{ metadata: any; data: any[] }>({ metadata: null, data: [] })
    ...
    >
      ...
      <SidebarInset>
        ...
        <div className="flex flex-1 flex-col gap-4 p-4 lg:gap-6 lg:p-6 bg-[#020617] text-slate-50 min-h-screen">
          ...
          <SmartButtons module="Eficiencia Operativa" data={{ totalSucursales: filteredData.length, avgEficiencia: stats?.avgEff?.toFixed(2), brechaTotal: stats?.totalGap?.toFixed(0), topSucursal: stats?.topBranch?.NOMBRE_SUCURSAL }} />
