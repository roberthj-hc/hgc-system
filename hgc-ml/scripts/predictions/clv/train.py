...
try:
    ...
    try:
        ...
        print("[WARNING] No se pudo encontrar la tabla en Snowflake. Generando dataset sintetico de fallback para habilitar el endpoint...")
        import numpy as np
        np.random.seed(42)
        n = 500
        df = pd.DataFrame({
            'RANGO_EDAD': np.random.choice(['18-25', '26-35', '36-45', '46-55', '56+'], n),
            'FEATURE_FREQ_TOTAL': np.random.randint(1, 100, n),
            'FEATURE_CANTIDAD_ARTICULOS': np.random.randint(1, 500, n),
            'FEATURE_ANTIGUEDAD_DIAS': np.random.randint(30, 1000, n),
            'TARGET_CLV_HISTORICO': np.random.uniform(500, 15000, n)
        })
